...
The browser calls same-origin `/api/{health,sample,scan,optimize}`; Next proxies to the private API.
The scan endpoint accepts multipart field `image`. Shared contracts live in `frontend/lib/types.ts`.
...
Run `python -m pytest backend/tests -q` from the repository root after installing backend/requirements-dev.txt.
...
Run `npm run test:e2e` against the running local services for full Playwright browser verification.
A real Gemini request still requires local credentials. Do not treat syntax checking or a static
screenshot as an end-to-end browser test. Current verification is recorded in docs/BUILD_PROGRESS.md.
